...
describe "example syntax declaring dataset" do
  ...
  before(:each) do
    dataset = EngageDatagen::Dataset.new do
      branch = branch(Name: "Blocker/Norfolk YMCA") do
  ...
  it "should create a branch" do
    bruce = dataset.find {|doc| doc[:FirstName] == "Bruce" }
    unit = dataset.find {|doc| doc.id == bruce[:Unit]}
    branch = dataset.find {|doc| doc.id == unit[:Membership][:Branch]}
    branch[:Name].should == "Blocker/Norfolk YMCA"
  end
